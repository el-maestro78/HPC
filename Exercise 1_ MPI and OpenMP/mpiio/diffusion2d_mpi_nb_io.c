...
{ // TODO: add your MPI I/O code here, write rho_ to disk
    ...
    MPI_Offset len = local_N_ * real_N_; // Number of elements to write
    MPI_Offset offset = rank * len * sizeof(double); // Offset for each rank in bytes

    // Write data from all processes into the file
    for (int i = 1; i <= local_N_; ++i) {
        MPI_File_write_at(out_file, offset, &rho_[i * real_N_], real_N_, MPI_DOUBLE, &status);
        offset += real_N_ * sizeof(double); // Move offset to the next row
    }
