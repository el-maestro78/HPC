...
{
    float *force;
    force = malloc((N+1)*sizeof(float));
    float *force2;
    force2 = malloc((N)*sizeof(float));
    float temp = 0.0f;
    force[-1]=0;
	float rm2 = rm * rm;
    /*
	for (size_t i=0; i<N; ++i) {
        ...
        //force = force + (12 * eps * (s6*s6 - s6) / r);
        temp = 12 * eps * (s6*s6 - s6) / r;
        force[i] = force2[i] + temp; // 0+temp, then force[i-1] + temp
        }
        return force[N-1];
        */
        for (size_t i=0; i<N; ++i) {
          ...
          float s6 = s2*s2*s2; // (rm/r)^6
          //force = force + (12 * eps * (s6*s6 - s6) / r);
          force[i] = 12 * eps * (s6*s6 - s6) / r;
          force2[i] = force[i-1];
	}
        //for(int i=0;i<N; i++)
          //temp = force2[i]+force[i];
        return force2[N-1]+force[N-1];
}
